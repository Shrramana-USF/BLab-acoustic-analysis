...
def save_analysis_to_box(
    ...
    # st.success(f"Analysis data saved to Box (session folder: {session_folder_id})")
...
def read_audio_bytes(raw: bytes):
    ...
    # try:
    #     # First try WAV/PCM via soundfile
    #     data, sr = sf.read(io.BytesIO(raw), always_2d=False)
    #     y = ensure_mono(np.asarray(data)).astype(np.float32)
    #     return y, sr
    # except Exception:
    #     # Fall back to MP3 (pydub + ffmpeg)
    #     audio = AudioSegment.from_file(io.BytesIO(raw))
    #     sr = audio.frame_rate
    #     y = np.array(audio.get_array_of_samples()).astype(np.float32)
    #     if audio.channels > 1:
    #         y = y.reshape(-1, audio.channels).mean(axis=1)
    #     y /= np.iinfo(audio.array_type).max  # normalize
    #     return y, sr
...
# def download_region_button(y_region: np.ndarray, sr: int, label: str, filename: str):
#     """Download button for cropped/selected region (forced mono)."""
#     if y_region is None or len(y_region) == 0:
#         return
#     y_mono = ensure_mono(y_region).astype(np.float32)
#     buf = io.BytesIO()
#     sf.write(buf, y_mono, sr, format="WAV")
